            ...
            try {
                ...
                if( qrCodeDocument && qrCodeDocument.length > 0 ) {
                    ...
                    [
                        {
                            value :
                            fs.createReadStream(`${PROJECT_ROOT_DIRECTORY}${process.env.QR_CODE_PATH}/${codeData.code}/index.html`),
                            options : {filename: '/index.html'}
                        },{
                            value : 
                            fs.createReadStream(`${PROJECT_ROOT_DIRECTORY}${process.env.QR_CODE_PATH}/${codeData.code}/image.png`),
                            options : { filename : '/image.png'
                        }
                    }];
                    
                    let pdfPath = 
                    `${PROJECT_ROOT_DIRECTORY}${process.env.QR_CODE_PATH}/${codeData.code}/qrCode.pdf`;
    ...
    static createHtmlFromEjs( data,code ) {
        return new Promise(async (resolve, reject) => {
            try {
                ...
                let qrCodePath = 
                `${PROJECT_ROOT_DIRECTORY}${process.env.QR_CODE_PATH}`;
                
                let pathToEjsFile = PROJECT_ROOT_DIRECTORY+"/template/qr-code/index.ejs";
    ...
    static upload( code ) {
        return new Promise(async (resolve, reject) => {
            try {
                ...
                `${PROJECT_ROOT_DIRECTORY}${process.env.QR_CODE_PATH}/${code}`;
                let urlPrefix = "";
