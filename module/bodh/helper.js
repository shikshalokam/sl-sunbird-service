        ...
        return new Promise(async (resolve, reject) => {
            try {
                ...
                for (let pointerToContentData = 0;
                    ...
                    if (addCourseToAutocomplete.statusCode != HTTP_STATUS_CODE["ok"].status && addCourseToAutocomplete.statusCode != 201) {
    ...
    static logQueryMissFromESAndBodh(queryString = "", searchServiceUrl = "") {
        return new Promise(async (resolve, reject) => {
            try {
                ...
                let filePath = PROJECT_ROOT_DIRECTORY + "/" + process.env.LOGGER_DIRECTORY + "/bodh/search";
    ...
    static createAutocompleteIndexMap(organisationId = "") {
        return new Promise(async (resolve, reject) => {
            try {
                ...
                if (createBodhContentIndex.statusCode != HTTP_STATUS_CODE["ok"].status) {
    ...
    static uploadScromContent(file, name, token, userId) {
        return new Promise(async (resolve, reject) => {
            try {
                ...
                let zipPathName = `${PROJECT_ROOT_DIRECTORY}${process.env.ZIP_PATH}`
                ...
                filesHelper.removeFolder(`${PROJECT_ROOT_DIRECTORY}${process.env.ZIP_PATH}`)
                return reject(error);
            }
        })
    }
